...
choice=$(printf "%s\n" "${options[@]}" | $dmenu_command -p "Power Profiles ($current_option)")
...
# Make sure the choice is a valid option like rofi does
if [[ ! " ${options[@]} " =~ " ${choice} " ]]; then
    notify-send "Power Profile" "Invalid option '$choice'" -u critical
    exit 0
fi
